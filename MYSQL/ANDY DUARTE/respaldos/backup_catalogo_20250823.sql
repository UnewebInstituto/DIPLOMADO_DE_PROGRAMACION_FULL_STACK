...
--
-- Host: localhost    Database: bd_catalago_andy
-- ------------------------------------------------------
...
CREATE TABLE `imagenes` (
  ...
  CONSTRAINT `imagenes_ibfk_1` FOREIGN KEY (`productos_id`) REFERENCES `productos` (`id`) ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
INSERT INTO `productos` VALUES (1,1,'Smartphone X',799.99,50,'Smartphone de ?ltima generaci?n con c?mara de alta resoluci?n.'),(2,2,'Camiseta de algod?n',19.50,200,'Camiseta 100% algod?n, disponible en varios colores.'),(3,3,'Set de sartenes antiadherentes',85.00,30,'Juego de sartenes de 3 piezas para cocina.'),(4,4,'El Quijote',15.75,150,'Edici?n de bolsillo de la obra maestra de Cervantes.'),(5,1,'Auriculares Bluetooth',45.90,100,'Auriculares inal?mbricos con cancelaci?n de ruido.');
...
INSERT INTO `productos_etiquetas` VALUES (2,1),(3,1),(1,2),(1,3),(3,4),(2,5);
...
/*!50001 VIEW `vista_productos` AS select `a`.`nombre` AS `Productos`,`a`.`precio` AS `precio`,`a`.`cantidad` AS `cantidad`,`b`.`nombre` AS `nombre`,`b`.`descripcion` AS `descripcion_CATEGORIA`,`c`.`nombre` AS `etiquetas`,`c`.`descripcion` AS `descripcion_ETIQUETAS` from (((`productos` `a` join `categorias` `b`) join `etiquetas` `c`) join `productos_etiquetas` `d`) where `a`.`categoria_id` = `b`.`id` and `d`.`producto_id` = `a`.`id` and `d`.`etiqueta_id` = `c`.`id` */;
...
-- Dump completed on 2025-08-23 15:55:53
