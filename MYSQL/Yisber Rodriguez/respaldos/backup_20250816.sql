...
--
-- Host: localhost    Database: bd_yisber_20250802
-- ------------------------------------------------------
...
INSERT INTO `contactos` VALUES (1,'ANA MARIA','VASQUEZ','AV@GMAIL.COM','+58 414 2839451','+58 212 3313331','+58 2123315252'),(2,'YOLANDA','TORTOZA','YT@GMAIL.COM','+58 414 7654321','+58 212 3215476','+58 212 1234567'),(3,'NELLY','CONTRERAS','NC@GMAIL.COM','+58 414 9874567','+58 212 4436543','+58 212 3364321');
...
INSERT INTO `productos1` VALUES (21,11,'LAVADORA',800.25,12),(22,11,'NEVERA',700.50,6),(23,11,'COCINA',300.75,8),(24,11,'AA',450.00,9),(25,11,'SECADORA',600.00,8),(31,3,'FREIDORA',280.25,8),(32,3,'PARRILLERA',200.50,6),(33,3,'LAVAPLATOS',350.75,8),(34,3,'MICRONDAS',250.00,12),(35,3,'NEVERA',450.00,9),(36,4,'LAVADORA',580.75,12),(37,4,'AA',500.50,3),(38,4,'SECADORA',650.75,4),(39,4,'MICRONDAS',350.00,6),(40,4,'NEVERA',350.75,4);
...
INSERT INTO `productos2` VALUES (1,1,'LAVADORA',800.25,12),(2,1,'NEVERA',700.50,6),(3,1,'COCINA',300.75,8),(4,1,'AA',450.00,9),(5,1,'SECADORA',600.00,8),(6,2,'LAVADORA',800.25,12),(7,2,'CONGELADOR',300.50,3),(8,2,'COCINA',290.75,4),(9,2,'VENTILADOR',150.00,6),(10,2,'SECADORA',750.00,8),(11,3,'FREIDORA',280.25,8),(12,3,'PARRILLERA',200.50,6),(13,3,'LAVAPLATOS',350.75,8),(14,3,'MICRONDAS',250.00,12),(15,3,'NEVERA',450.00,9),(16,4,'LAVADORA',580.75,12),(17,4,'AA',500.50,3),(18,4,'SECADORA',650.75,4),(19,4,'MICRONDAS',350.00,6),(20,4,'NEVERA',350.75,4),(21,9,'LAVADORA',580.75,12),(22,9,'AA',500.50,3),(23,9,'SECADORA',650.75,4),(24,9,'MICRONDAS',350.00,6),(25,9,'NEVERA',350.75,4);
...
INSERT INTO `proveedores` VALUES (1,'GENERAL ELECTRIC','AV. BARALT','+58 212 1234567','info@ge.com'),(2,'WHIRPOOL','AV. SAN MARTIN','+58 414 9876543','info@whirpool.com'),(3,'MABE','AV. ROMULO GALLEGOS','+58 212 7654321','info@mabe.com'),(4,'LG','AV. ROMULO GALLEGOS','+58 212 6543212','info@lg.com'),(10,' SONIC','AV. VICTORIA','+58 212 1234567','info@sonic.com'),(11,'PHILPHS','AV. URDANETA','+58 414 9876543','info@philiphs.com'),(12,'ZENITH','AV. NVA GRANADA','+58 212 6543212','info@zenith.com');
...
INSERT INTO `proveedores1` VALUES (3,'LG','AV. ROMULO GALLEGOS','+58 212 6543212','info@lg.com'),(4,'MABE','AV. FRANCISCO DE MIRANDA','+58 212 7654321','info@mabe.com'),(11,'GENERAL ELECTRIC','AV. BARALT','+58 212 1234567','info@ge.com');
...
--
-- Temporary table structure for view `vista_prov_prod_letf_join`
--

DROP TABLE IF EXISTS `vista_prov_prod_letf_join`;
/*!50001 DROP VIEW IF EXISTS `vista_prov_prod_letf_join`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE VIEW `vista_prov_prod_letf_join` AS SELECT
 1 AS `nombre`,
  1 AS `direccion`,
  1 AS `telefono`,
  1 AS `correo`,
...
/*!50001 VIEW `vista_prov_prod` AS select `a`.`nombre` AS `nombre`,`a`.`direccion` AS `direccion`,`a`.`telefono` AS `telefono`,`a`.`correo` AS `correo`,`b`.`nombre` AS `producto`,`b`.`precio` AS `precio`,`b`.`existencia` AS `existencia` from (`proveedores` `a` join `productos` `b`) where `b`.`proveedor_id` = `a`.`id` */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `vista_prov_prod_inner_join` AS select `a`.`nombre` AS `nombre`,`a`.`direccion` AS `direccion`,`a`.`telefono` AS `telefono`,`a`.`correo` AS `correo`,`b`.`nombre` AS `producto`,`b`.`precio` AS `precio`,`b`.`existencia` AS `existencia` from (`proveedores` `a` join `productos` `b`) where `b`.`proveedor_id` = `a`.`id` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `vista_prov_prod_letf_join`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `vista_prov_prod_letf_join` AS select `a`.`nombre` AS `nombre`,`a`.`direccion` AS `direccion`,`a`.`telefono` AS `telefono`,`a`.`correo` AS `correo`,`b`.`nombre` AS `producto`,`b`.`precio` AS `precio`,`b`.`existencia` AS `existencia` from (`proveedores` `a` join `productos` `b`) where `b`.`proveedor_id` = `a`.`id` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `vista_resumen_proveedores` AS select distinct `vista_prov_prod`.`nombre` AS `nombre`,min(`vista_prov_prod`.`precio`) AS `precio_minimo`,max(`vista_prov_prod`.`precio`) AS `precio_maximo`,round(avg(`vista_prov_prod`.`precio`),2) AS `precio_promedio`,sum(`vista_prov_prod`.`existencia`) AS `existencia` from `vista_prov_prod` group by `vista_prov_prod`.`nombre` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2025-08-16 16:49:51
