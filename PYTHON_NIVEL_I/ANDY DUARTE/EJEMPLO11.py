...
while True:
    ...
    fecha_ingreso =input("FECHA INGRESO(aaaa-m-dd):")
    while True:
        try:    
    ...
    while True:
        empresa=input("NUMERO DE EMPRESA (1 O 2)")
        if empresa in ('1','2'):
            break
        else:
            print("ERROR:Debe ingresar 1 o 2 para el numero de empresa")
    ...
    if empresa =='1':
        ...
        deduccion_sso1 += deduccion_sso
    else:
        deduccion_sso = monto_base_imponible *0.114
        deduccion_sso2+=deduccion_sso
    ...
    print("TOTAL PAGO TRABAJADORES:{:.2f}".format(total_pago))
